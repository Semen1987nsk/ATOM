...
def calculate_sharpe_sortino(trades_pnl: List[float], risk_free_rate: float = 0.0) -> Dict:
    ...
    downside_diff = np.minimum(returns - risk_free_rate, 0)
    downside_dev = np.sqrt(np.mean(np.square(downside_diff)))
    sortino = (mean_return - risk_free_rate) / downside_dev if downside_dev > 0 else 99.99
