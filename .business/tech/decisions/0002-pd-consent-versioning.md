...
- ~~`finalize_deletion` должна вызываться регулярно — нужен scheduler (пока не подключён, ручной запуск через `pd_deletion.run_pending_deletions(db)`)~~ ✅ Закрыто 2026-05-07: подключено к `sync_scheduler.SyncScheduler._check_pd_finalizations()` с интервалом 24 часа.
...
- Реализация Phase 2 — `backend/sync_scheduler.py:_check_pd_finalizations()` (24h интервал, asyncio thread). Подключено 2026-05-07. Если впоследствии перейдём на Celery / cron — отдельный ADR.
