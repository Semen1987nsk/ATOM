...
#
# Usage (cron):
#   0 3 * * * /opt/eqio/scripts/backup_db.sh >> /var/log/eqio/backup.log 2>&1
#
# Expected env vars:
#   DATABASE_URL      — postgres://user:pass@host:5432/eqio_prod
#   BACKUP_DIR        — target directory (default /var/lib/eqio/backups)
...
#
# Output:
#   - <BACKUP_DIR>/eqio-YYYYMMDD-HHMMSS.sql.gz
#   - (если BACKUP_S3_BUCKET задан) s3://<bucket>/eqio-YYYYMMDD-HHMMSS.sql.gz
...
FILENAME="eqio-${TIMESTAMP}.sql.gz"
...
find "${BACKUP_DIR}" -name "eqio-*.sql.gz" -mtime "+${RETENTION_DAYS}" -delete
