# Empirik Stack — Tools, MCP-серверы, Скиллы
...
Скиллы лежат в **`.claude/skills/`** репозитория Empirik (командные, в git).
Активируются автоматически по триггерам в описании.

### Скиллы для Empirik (написаны под этот проект)
...
Из набора Anthropic — используются в Empirik при необходимости.
...
- **Скиллы не активны на разработческих сессиях, не открывших Empirik** — потому что они в `.claude/skills/` репо, а не в `~/.claude/skills/`. При работе вне репо нужно либо открыть Empirik, либо скопировать в personal-папку.
...
### Месяц 3
- Написать `playwright-vitest-empirik-e2e` skill (когда тестов фронта будет ≥30)
